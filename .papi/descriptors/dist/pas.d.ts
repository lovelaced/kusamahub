...
import { I5sesotjlssv2d, I21d2olof7eb60, Ia0v3r2qmg79ve } from "./common-types";
...
export type PasDispatchError = unknown;
type PalletsTypedef = {
    __storage: IStorage;
    __tx: ICalls;
    __event: IEvent;
...
export type PasViewFns = ViewFnsFromPalletsDef<PalletsTypedef>;
export type PasCallData = Anonymize<Ia0v3r2qmg79ve> & {
    value: {
        type: string;
    };
};
export type PasWhitelistEntry = PalletKey | ApiKey<IRuntimeCalls> | `query.${NestedKey<PalletsTypedef['__storage']>}` | `tx.${NestedKey<PalletsTypedef['__tx']>}` | `event.${NestedKey<PalletsTypedef['__event']>}` | `error.${NestedKey<PalletsTypedef['__error']>}` | `const.${NestedKey<PalletsTypedef['__const']>}` | `view.${NestedKey<PalletsTypedef['__view']>}`;

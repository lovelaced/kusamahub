...
import { I5sesotjlssv2d, I21d2olof7eb60, I766dsrsdk5kal } from "./common-types";
...
export type KsmDispatchError = unknown;
type PalletsTypedef = {
    __storage: IStorage;
    __tx: ICalls;
    __event: IEvent;
...
export type KsmCallData = Anonymize<I766dsrsdk5kal> & {
    value: {
        type: string;
    };
};
export type KsmWhitelistEntry = PalletKey | ApiKey<IRuntimeCalls> | `query.${NestedKey<PalletsTypedef['__storage']>}` | `tx.${NestedKey<PalletsTypedef['__tx']>}` | `event.${NestedKey<PalletsTypedef['__event']>}` | `error.${NestedKey<PalletsTypedef['__error']>}` | `const.${NestedKey<PalletsTypedef['__const']>}` | `view.${NestedKey<PalletsTypedef['__view']>}`;

...
contract PixelCanvas {
    ...
    uint256 public constant RECIPIENT_PERCENTAGE = 51;
    ...
    function placePixelsBatch(
        uint256[] calldata pixelIds,
        ...
        for (uint256 i = 0; i < pixelIds.length; i++) {
            ...
            bool wasOwned = pixels[pixelIds[i]].owner == msg.sender;
    ...
    function getPixel(uint256 pixelId) external view validPixelId(pixelId) returns (
        address pixelOwner,
        uint32 color,
        uint256 lastModified
    ) {
    ...
    function getPixelsByRange(
        ...
        require(startId + count <= TOTAL_PIXELS, "Range exceeds canvas");
        require(count <= 1000, "Range too large");
        
        owners = new address[](count);
        colors = new uint32[](count);
        timestamps = new uint256[](count);
        
        for (uint256 i = 0; i < count; i++) {
            Pixel memory pixel = pixels[startId + i];
            owners[i] = pixel.owner;
            colors[i] = pixel.color;
            timestamps[i] = pixel.lastModified;
        }
    }
